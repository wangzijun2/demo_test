...
class CreateApproval:
    ...
    def save_approval_dtl(self):
        ...
        approval_dtl_params = eval(data_approval.get_approval_dtl())
        # 获取采购协议的商品和门店
        ...
        if logistics_type == '1':
            ...
            if logistics_sign == str(1):
                for i in range(len(item_num_id_list)):
                    if len(item_num_id_list) <= 3:
                        package_qty = input('请输入%s的商品配送件数:' % item_num_id_list[i])
                    else:
                        package_qty = 100
                    conversion_qty = self.get_approval_item_conversion_qty(1000049, item_num_id_list[i])
                    if conversion_qty:
                        approval_dtl_params['item_num_id'] = item_num_id_list[i]
                        approval_dtl_params['ord_sub_unit_num_id'] = 1000049
                        ...
                        data_request = DataRequest()
                        req = requests.post(data_request.get_data()['url'], data_request.get_data())
                        print('商品:', item_num_id_list[i], '门店:', 1000049, '保存:', req.json()['message'], 'series行号:',
                              req.json()['series'])
            # 门店直送
    ...
    def approval_audit(self):
        ...
        data_approval = DataApprovalParm()
        approval_num_id = self.get_approval_num_id()
        approval_params['sub_unit_num_id'] = 1000049
